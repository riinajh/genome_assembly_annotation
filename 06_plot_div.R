...
ggplot(rep_table.m, aes(fill = fam, x = age, weight = value / 1000000)) +
  ...
  xlab("TE age (MYA)") +
  ylab("Amount of sequence (Mbp)") +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, size = 9, hjust = 1), plot.title = element_text(hjust = 0.5))

ggplot(rep_table.m, aes(fill = fam, x = age, y = fam, weight = value / 10000000))+
  geom_violin()
ggsave(filename = "TE_age_all_violins.pdf", width = 10, height = 5, useDingbats = F)
